...
class NMRObject():
    ...
    def remove_baseline(self, poly_order=24):
        ...
        x = np.arange(0, self.np)
        baseline_idx = list(set(x) - set(self.peak_idx))
        self.line_broad(self.lb)
        y = np.real(self.phased_spectrum)
        p = np.polyfit(x[baseline_idx], y[baseline_idx], poly_order)
        self.baseline = np.polyval(p, x)
        self.clean_spectrum = y -self.baseline
    ...
    def __dir__(self):
        return ("ph0", "ph1", "ph", "lb", "np", "sw", "fid", "fidlb", "spectrum", "phased_spectrum", "baseline")
...
class EXPbject():
    ...
    def line_broad(self, lb=20):
        idx = np.linspace(0, self.np, self.np)
        self.lb = lb
        self.fidlb = self.fid * np.exp(-self.lb*idx/1000)
        self.spectrum = fftshift(fft(self.fidlb))
        self.phase(self.ph0, self.ph1)
    ...
    def __dir__(self):
        return ("ph0", "ph1", "ph", "lb", "np", "fid", 
                "fidlb", "spectrum", "phased_spectrum", "baseline") 